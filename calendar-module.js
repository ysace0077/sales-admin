...
class CalendarModule {
    ...
    render() {
        ...
        container.innerHTML = `
            ...
            <div style="background: white; border-radius: 15px; padding: 20px; box-shadow: 0 5px 20px rgba(0,0,0,0.1);">
                <div class="calendar-controls" style="background: #f8f9fa; padding: 20px; border-radius: 10px; margin-bottom: 20px;">
                    <div class="calendar-filters" style="display: flex; gap: 15px; flex-wrap: wrap; align-items: center; justify-content: space-between;">
                        ...
                        <div class="calendar-stats" style="display: flex; gap: 10px; align-items: center; padding: 15px; background: white; border-radius: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                            <div style="text-align: center; padding: 0 15px; border-right: 2px solid #e9ecef;">
                                <div class="stat-label" style="font-size: 0.8em; color: #6c757d; margin-bottom: 5px; font-weight: 600;">일간 평균</div>
                                <div class="stat-value" id="dailyAvg" style="font-size: 1.1em; font-weight: 700; color: #28a745;">0원</div>
                            </div>
                            <div style="text-align: center; padding: 0 15px;">
                                <div class="stat-label" style="font-size: 0.8em; color: #6c757d; margin-bottom: 5px; font-weight: 600;">월간 매출</div>
                                <div class="stat-value" id="monthlySales" style="font-size: 1.1em; font-weight: 700; color: #28a745;">0원</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <table class="calendar-table" style="width: 100%; table-layout: fixed; border-collapse: collapse; margin-bottom: 20px;">
                    <thead>
                        <tr>
                            ${['월', '화', '수', '목', '금', '토', '일'].map(day => 
                                `<th style="width: 14.28%; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 15px; text-align: center; font-weight: 600; border: 1px solid #e0e6ed;">${day}</th>`
                            ).join('')}
                        </tr>
                    </thead>
                ...
                <div id="dailySalesDetails" style="display: none; margin-top: 30px;">
                    ...
                    <div class="daily-sales-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; padding: 20px; background: #f8f9fa; border-radius: 10px;">
                    ...
                    <div id="dailySalesCards" class="daily-sales-cards" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(400px, 1fr)); gap: 20px; background: #f8f9fa; padding: 20px; border-radius: 10px;">
                    </div>
                </div>
            </div>
        `;
    ...
    loadSellers() {
        try {
            ...
            if (this.currentCompany) {
                ...
                const result = this.db.exec(`SELECT DISTINCT 판매자 FROM "${tableName}" WHERE 판매자 IS NOT NULL AND 판매자 != '' ORDER BY 판매자`);
            ...
            // 중복 제거된 판매자 목록을 정렬하여 추가
            Array.from(uniqueSellers).sort().forEach(seller => {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            html += `
                <td style="border: 1px solid #e0e6ed; padding: 8px; text-align: center; vertical-align: top; height: 95px; background: ${bgColor}; cursor: pointer; transition: all 0.2s ease;" 
                    ...
                    <div class="calendar-date" style="font-weight: 700; font-size: 16px; margin-bottom: 6px; color: ${dateColor};">${day}</div>
                    <div class="calendar-amount" style="font-size: 12px; font-weight: 700; padding: 3px 6px; border-radius: 5px; ${
    ...
    updateStats() {
        ...
        // 모바일 환경에서는 백만원 단위로 표기
        const isMobile = window.innerWidth <= 768;
        
        if (isMobile) {
            // 일간 평균: 소수점 1자리까지, 이하는 roundup
            const avgMillionValue = avgSales / 1000000;
            const roundedAvg = Math.ceil(avgMillionValue * 10) / 10;
            
            // 월간 매출: 소수점 1자리까지, 이하는 roundup
            const totalMillionValue = totalSales / 1000000;
            const roundedTotal = Math.ceil(totalMillionValue * 10) / 10;
            
            document.getElementById('dailyAvg').textContent = `${roundedAvg.toFixed(1)}백만원`;
            document.getElementById('monthlySales').textContent = `${roundedTotal.toFixed(1)}백만원`;
        } else {
    ...
    formatAmount(amount) {
        ...
        // 모바일 환경에서는 간소화된 표기 사용
        const isMobile = window.innerWidth <= 768;
        
        if (isMobile) {
            // 모바일: 백만원 단위로 표기, 소수점 1자리까지, 이하는 roundup
            const millionValue = absAmount / 1000000;
            
            // Math.ceil을 사용하여 소수점 1자리에서 roundup
            const roundedValue = Math.ceil(millionValue * 10) / 10;
    ...
    createCustomerCard(group) {
        ...
        let cardHTML = `
            <div class="customer-card" style="background: white; border: none; border-radius: 16px; padding: 20px; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); position: relative; overflow: hidden; box-shadow: 0 2px 12px rgba(0, 0, 0, 0.06); border-left: 5px solid #667eea;">
                <style>
                    @media (max-width: 768px) {
                        .customer-card {
                            padding: 12px !important;
                ...
                <div class="customer-header" style="margin-bottom: 16px; padding-bottom: 12px; border-bottom: 2px solid #e9ecef;">
                    <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
                        <div style="font-size: 1.2em; font-weight: 700; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;">
                            ${records[0].index}.
                        </div>
                        <div class="customer-title" style="font-size: 1.1em; font-weight: 700; color: #212529;">
                            ${customerName}
                        </div>
                        ${locationInfo ? `<div class="customer-location" style="font-size: 0.8em; color: #6c757d; padding: 4px 8px; background: #e9ecef; border-radius: 6px; font-weight: 600;">${locationInfo}</div>` : ''}
                    </div>
                </div>
                <div class="customer-info" style="display: flex; flex-direction: column; gap: 4px; font-size: 0.9em; line-height: 1.3;">
        `;
        ...
        if (firstRecord['판매일']) {
            ...
            cardHTML += `
                <div class="info-row">
                    <span class="info-label">판매일:</span><span class="info-value" style="color: #212529;">${formattedDate}</span>
                </div>
            `;
        }
        ...
        if (firstRecord['판매자']) {
            cardHTML += `
                <div class="info-row">
                    <span class="info-label">판매자:</span><span class="info-value" style="color: #212529; font-weight: 600;">${firstRecord['판매자']}</span>
                </div>
            `;
        }
        ...
        if (firstRecord['배송일'] || firstRecord['배송예정일']) {
            ...
            cardHTML += `
                <div class="info-row" style="margin-bottom: 8px;">
                    <span class="info-label">배송일:</span><span class="info-value" style="color: #212529;">${formattedDeliveryDate}</span>
                </div>
            `;
        }
        ...
        records.forEach((record, index) => {
            if (index > 0) {
                cardHTML += `<div class="product-separator"></div>`;
            }
            ...
            if (productInfo) {
                cardHTML += `
                    <div class="info-row">
                        <span class="info-label">상품:</span><span class="info-value product-info-long" style="color: #667eea; font-weight: 600;">${productInfo}</span>
                    </div>
                `;
            }
            ...
            if (record['구매용도']) {
                cardHTML += `
                    <div class="info-row">
                        <span class="info-label">구매용도:</span><span class="info-value" style="color: #212529;">${record['구매용도']}</span>
                    </div>
                `;
            }
            ...
            if (record['할인가'] || record['출고가']) {
                ...
                cardHTML += `
                    <div class="info-row">
                        <span class="info-label">금액:</span><span class="info-value" style="color: #10b981; font-weight: 700; font-family: 'SF Mono', Monaco, 'Courier New', monospace;">
                            ${parseInt(amount).toLocaleString()}원
                        </span>${isDiscounted ? '<span style="color: #ef4444; font-weight: 600; margin-left: 4px;">(할인가)</span>' : ''}
                    </div>
                `;
            }
            ...
            if (paymentInfo) {
                cardHTML += `
                    <div class="info-row">
                        <span class="info-label">결제:</span><span class="info-value" style="color: #212529;">${paymentInfo}</span>
                    </div>
                `;
            }
            ...
            if (discountRate) {
                cardHTML += `
                    <div class="info-row">
                        <span class="info-label">할인율:</span><span class="info-value" style="color: #f97316; font-weight: 700; font-family: 'SF Mono', Monaco, 'Courier New', monospace;">${discountRate}</span>
                    </div>
                `;
            }
        });
